...
def test_employee_ids(db_conn, employee_ids):
    ...
    assert len(result) == len(employee_ids), "The number of employee ids does not match the expected count."

    # Assert that all employee ids in the result
    # are in the employee_ids fixture
    for row in result:
        assert row[0] in employee_ids, f"Employee id {row[0]} not found in employee_ids fixture."
...
def test_employee_event_counts(db_conn, employee_event_counts):
    ...
    # Assert that the number of rows in the result matches the length of event_counts fixture
    assert len(result) == len(employee_event_counts), "The number of rows in the result does not match the expected count."
    ...
    for idx, row in enumerate(result.itertuples(index=False)):
        assert (row.event_date, row.positive_events, row.negative_events) == employee_event_counts[idx], f"Row {idx} does not match expected values."    
